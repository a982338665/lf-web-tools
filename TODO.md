...
已完成-二维码&条形码：帮我新增一个菜单【二维码&条形码】并实现相关功能，新增菜单的主页面跟其他菜单主页面（例如：菜单【公网端口检测】的主页面）保持整体样式一致（要跟其他菜单保持主体一致
已完成-证件照生成工具：帮我新增一个菜单【证件照生成工具】并实现相关功能，新增菜单的主页面跟其他菜单主页面（例如：菜单【公网端口检测】的主页面）保持整体样式一致（要跟其他菜单保持主体一致）
--============================================================================================================================
TODO：
...
033596666666666
=============================================================================================================================
curl命令测试那里，添加一个功能，保存测试历史，并且全局设置可以一键更改域名，便于测试和复现

1.帮我在右上角系统正常旁边添加登录注册功能（要求要有验证码，并且验证登录必须跟验证码一起提交，示例如图所示
），这个页面作为欢迎页面，除了Socket测试网页菜单，CURL测试网页，公网端口检测这三个菜单外，其他菜单功能都可以使用，并且未这三个菜单上把小锁，只有登录成功后才对登录用户开放。
2.帮我实现换肤功能，支持暗色，亮色，护眼，科技（科技色就是目前这个颜色），默认暗色
3.帮我换下logo，如上h5代码

注册功能主要字段：账号，密码，邮箱，手机号，图形验证码，点击注册，每个用户都应该分配一个uuid，注册成功后跳转到登录页面。
